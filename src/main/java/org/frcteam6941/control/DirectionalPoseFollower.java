    ...
    public Optional<HolonomicDriveSignal> update(
        ...
        if (targetPose == null) {
            ...
            if (targetPose.isXRestricted()) {
                x = xController.calculate(mDrivebase.getLocalizer().getLatestPose().getX(), targetPose.getX());
            }
            if (targetPose.isYRestricted()) {
                y = yController.calculate(mDrivebase.getLocalizer().getLatestPose().getY(), targetPose.getY());
            }
            return Optional.of(
