...
public class FieldObstacles {
        private static double margin = Constants.SUBSYSTEM_DRIVETRAIN.DRIVETRAIN_SIDE_WIDTH_BUMPERED * 0.5 + 0.20;
        ...
        public static Obstacle boundaryChargingStation = new RectangularObstacle(
                        List.of(FieldConstants.Community.chargingStationCorners).stream()
                                        .map(translation -> FieldConstants.allianceFlip(translation))
                                        .collect(Collectors.toList()).toArray(new Translation2d[0]),
                        margin, margin);

        public static Obstacle[] obstacleList = new Obstacle[] { boundaryXSmall, boundaryXLarge, boundaryYSmall,
                        boundaryYLarge, boundaryChargingStation };
}
